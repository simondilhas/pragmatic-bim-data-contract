...
def _extract_attributes(el):
    attrs = {}
    for attr in getattr(el.wrapped_data, "get_attr_names", lambda: [])():
        try:
            val = getattr(el, attr)
        except Exception as e:
            logger.warning(f"Skipping attribute {attr!r} on element {el.id()}—{e!r}")
            continue
        if not callable(val):
            attrs[attr] = val
    return attrs
...
def _extract_attributes(el):
    ...
    try:
        ...
        logger.warning(f"Failed to extract attributes for element {el.id()}—{e!r}")
